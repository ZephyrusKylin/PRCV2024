...
def split_image_into_patches(img, mask, patch_size):
    img_patches = []
    mask_patches = []
    ...
    # 计算需要填充的行和列数
    pad_row = patch_size - h % patch_size if h % patch_size != 0 else 0
    pad_col = patch_size - w % patch_size if w % patch_size != 0 else 0
    
    # 填充图像
    img = np.pad(img, ((0, pad_row), (0, pad_col)), mode='constant')
    mask = np.pad(mask, ((0, pad_row), (0, pad_col)), mode='constant')
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            mask_patch = mask[i*patch_size:(i+1)*patch_size, j*patch_size:(j+1)*patch_size]
...
def get_img_norm_cfg(dataset_name, dataset_dir):
    if  dataset_name == 'NUAA-SIRST':
    ...
    elif dataset_name == 'WildIRSTD':
        img_norm_cfg = dict(mean=44.12226104736328, std=20.91413688659668)
    elif dataset_name == 'WildIRSTD_test':
        ...
        # with open(dataset_dir + '/' + dataset_name +'/val.txt', 'r') as f:
        #     test_list = f.read().splitlines()
        # img_list = train_list + test_list
        img_list = train_list
        img_dir = dataset_dir + '/' + dataset_name + '/train/img/'
        mean_list = []
        std_list = []
...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    if optimizer_name == 'Adam':
        ...
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=optimizer_settings['lr'])
...
if __name__ == '__main__':
    name='WildIRSTD'
    path='/home/Newdisk/wangzhang'
    get_img_norm_cfg(name,path)
